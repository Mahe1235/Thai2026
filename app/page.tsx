...
function SectionLabel({ label, href }: { label: string; href?: string }) {
  return (
    <div className="flex items-center justify-between mb-2.5">
      ...
      {href && (
        <Link href={href} className="text-[11px] text-muted-foreground hover:text-foreground transition-colors">
          See all
        </Link>
      )}
    </div>
      ...
      <CardContent className="flex items-center gap-3 p-4">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-3 mt-0.5">
            <span className="flex items-center gap-1 text-xs text-muted-foreground"><Droplets size={11} /> {w.humidity}%</span>
            <span className="flex items-center gap-1 text-xs text-muted-foreground"><Wind size={11} /> {w.windSpeed} km/h</span>
            <span className="flex items-center gap-1 text-xs text-muted-foreground"><MapPin size={11} /> {w.location}</span>
          </div>
        </div>
      </CardContent>
    </Card>
      ...
      <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.14 }}>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            { label: 'Pool spent', val: stats ? fmtBaht(stats.spent)     : null, color: 'text-primary' },
            { label: 'Split',      val: stats ? fmtBaht(stats.splitTotal): null, color: 'text-foreground' },
          ].map(item => (
            <Link key={item.label} href="/expenses">
              <Card className="p-3.5 text-center hover:border-primary/30 active:bg-secondary transition-colors cursor-pointer">
                {item.val
                  ? <div className={`font-mono text-[15px] font-bold ${item.color}`}>{item.val}</div>
                  : <div className="skeleton h-5 w-14 mx-auto rounded" />
                }
                <div className="text-[10px] text-muted-foreground mt-1 tracking-wide">{item.label}</div>
              </Card>
            </Link>
          ))}
        </div>
            ...
            <Card className="p-4 hover:border-primary/40 transition-colors cursor-pointer">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-mono text-sm font-bold text-primary">{nextFlight.flight}</span>
                  <span className="text-xs text-muted-foreground">{nextFlight.airline}</span>
                </div>
              ...
              <div className="flex items-center gap-2">
                <div className="text-center">
                  ...
                  <div className="text-[11px] text-muted-foreground">{nextFlight.fromFull}</div>
                </div>
                ...
                <div className="text-center">
                  ...
                  <div className="text-[11px] text-muted-foreground">{nextFlight.toFull}</div>
                </div>
              </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <Link key={href} href={href}>
              <Card className="flex flex-col gap-2.5 p-4 hover:border-primary/25 active:bg-secondary transition-colors cursor-pointer h-full">
